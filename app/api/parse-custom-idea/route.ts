...
function normalizeContentType(value: unknown) {
  ...
  if (text.includes("UGC") || text.includes("VIDEO AD")) return "UGC VIDEO"
...
function splitFallbackIdeas(inputText: string) {
  const blocks = inputText
    .split(/\n(?=(?:Static\s+\d+|UGC\s+Video|Video\s+Ad|Album(?:\s+Ad)?|Motion(?:\s+Ad)?|Short\s+(?:VDO|Video)|Option\s+[A-Z])\b)/gi)
...
- Map Static to STATIC AD, UGC/Video Ad to UGC VIDEO, Album to ALBUM AD, and Motion/Short video to SHORT VDO.
