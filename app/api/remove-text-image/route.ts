...
export async function POST(request: Request) {
  try {
    ...
    const sourceAspectRatio =
      typeof body.source_aspect_ratio === "string" && SUPPORTED_ASPECT_RATIOS.includes(body.source_aspect_ratio as any)
        ? body.source_aspect_ratio
        : null
    const targetSize =
      SUPPORTED_IMAGE_SIZES.find((size) => size === body.target_size) || "1K"
    ...
    try {
      ...
      console.error("[remove-text-image] Failed to parse Gemini response:", error, responseText)
    ...
    const parts = geminiPayload?.candidates?.flatMap((candidate: any) => candidate?.content?.parts || []) || []
    ...
    if (!imageBase64) {
      console.error("[remove-text-image] No image returned from Gemini:", geminiPayload)
      return NextResponse.json(
        { success: false, error: "Gemini did not return a text-removed image", details: responseMessage },
        { status: 500 },
      )
    }
    ...
    const outputAspectRatio = outputDimensions ? getClosestAspectRatio(outputDimensions.width, outputDimensions.height) : null

    if (outputAspectRatio && outputAspectRatio !== aspectRatio) {
      return NextResponse.json(
        {
          success: false,
          error: `Remove text output ratio changed from ${aspectRatio} to ${outputAspectRatio}`,
          requested_aspect_ratio: aspectRatio,
          actual_aspect_ratio: outputAspectRatio,
          actual_dimensions: outputDimensions,
    ...
    return NextResponse.json({
      ...
      aspect_ratio: outputAspectRatio || aspectRatio,
    ...
    console.error("[remove-text-image] Unexpected error:", error)
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Failed to remove text from image",
      },
      { status: 500 },
    )
  }
